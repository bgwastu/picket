...
// UninstallAgent requests removal of the Picket systemd installation.
func (sys *System) UninstallAgent() error {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	var result common.UninstallAgentResponse
	if err := sys.request(ctx, common.UninstallAgent, nil, &result); err != nil {
		return err
	}
